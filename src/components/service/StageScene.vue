...
import { rng } from './rng'
import * as fwNear from './fireworksNear'
import * as fwFar from './fireworksFar'
...
const scene = computed(() => {
  ...
  const ripples = s.river
    ? Array.from({ length: 8 }, (_, i) => ({
        i,
        y: fwGeo.value.rip0 + i * fwGeo.value.ripStep + pick(-3, 3),
        o: pick(0.05, 0.15),
        dur: pick(9, 18),
        delay: -pick(0, 9),
      }))
 ...
 * 불꽃놀이 두 판.
 *
 * 같은 한강 둔치를 서로 다른 거리에서 본다. 멀리(far)는 2026-08-27 에
 * 세운 판이고, 가까이(near)는 그 다음 날 물을 줄여 하늘을 넓히고 공을
 * 키우고 터지는 순간의 번쩍임을 둔 판이다.
 *
 * 쇼의 짜임도 알갱이 물리도 서로 다른 것이라 값 하나로 갈라 쓰지 않고
 * 판마다 한 벌씩 따로 둔다. 여기서 하는 일은 무대가 말하는 쪽(fwShow)을
 * 골라 그 판의 눈금과 그림을 받아 오는 것뿐이다.
 */
const FW_SHOWS = { near: fwNear, far: fwFar }
const fwShow = computed(() => FW_SHOWS[props.stage.fwShow] ?? FW_SHOWS.near)
/*
 * 강가 판의 y 값은 전부 수면 하나에 매달려 있다 — 도시 밑동도 다리도
 * 가로등도 물결도. 그래서 그림이 숫자를 직접 들지 않고 판에서 받는다.
 */
const fwGeo = computed(() => fwShow.value.GEO)
...
const bfRun = (now) => {
  ...
  if (props.stage.fireworks) fwState.value = fwShow.value.step(time, frame, fwMirrorOn)
  ...
  if (props.open && wants && !still) {
    ...
    if (still) {
      ...
      if (props.stage.fireworks) {
        ...
        fwShow.value.reset()
        for (let i = 0; i < 190; i++)
          fwState.value = fwShow.value.step(i * 0.016, 0.016, fwMirrorOn)
      }
    }
  }
}
...
<template>
  ...
  >
    <defs>
      ...
      -->
      <radialGradient
        v-for="(pl, pi) in fwShow.PALETTES"
        :id="`fwblast${pi}-${uid}`"
        :key="`fwb${pi}`"
      >
        <stop offset="0%" :stop-color="fwShow.CORE" stop-opacity="1" />
        ...
        <g :fill="stage.lampNeon">
          <template v-for="b in scene.city" :key="`cc${b.i}`">
            <circle
              v-if="b.crown"
              :cx="b.x + b.w / 2"
              :cy="fwGeo.water - b.h - 1.6"
              r="1.15"
              opacity="0.5"
            />
          </template>
          <circle cx="598" :cy="fwGeo.beaconCy" r="1.3" opacity="0.6" />
        </g>
      </g>
    ...
    <!--
      ...
      가까이 보는 강 판에는 없다. 이 판에서 지평선을 만드는 건 능선이
      아니라 강 건너 불빛이다.
      ...
      멀리서 보는 강가 판(fwGeo.hills)에서는 그대로 켜 둔다. 거기서는
      수면이 아직 y140 이라 능선이 여전히 물 밑에 있다.
    -->
    <path
      v-if="!stage.beach && (!stage.river || fwGeo.hills)"
      :fill="stage.far"
    ...
    <!--
      ...
      가까이 보는 강 판에는 가를 원경이 없다 — 도시 바로 밑이 물이다.
    -->
    <rect
      v-if="!stage.beach && (!stage.river || fwGeo.hills)"
      x="0"
    ...
    <g v-if="stage.river" class="river">
      ...
      <!--
        ...
        누르는 정도는 판마다 다르다. 물이 좁게 보일수록 더 눕혀 본다.
      -->
      <g
        ...
        fill="none"
        stroke-linecap="round"
        :transform="`translate(0 ${fwGeo.water * (1 + fwGeo.mirrorK)}) scale(1 -${fwGeo.mirrorK})`"
      >
        <path
      ...
      <g class="bridge">
        <g :fill="stage.bridgeDark">
          <rect
            ...
            :height="fwGeo.pierH"
          />
          <rect x="-20" :y="fwGeo.deckY" width="840" height="8" />
          <rect x="-20" :y="fwGeo.railY" width="840" height="2.4" :fill="stage.bridgeRail" />
        </g>
    ...
    <rect
      ...
      width="800"
      height="260"
      :fill="fwGeo.litGrad ? `url(#fwlit-${uid})` : stage.lampWarm"
