    ...
    <g class="lit">
      ...
      -->
        ...
        <!--
          ...
          접힌 우산을 든 사람(hold === 1)이 비를 만나면 그걸 편다.
          그때 접힌 것을 지우지 않았더니 우산이 둘이 됐다 —
          펴 든 우산 하나와 여전히 손에 들린 접힌 우산 하나.
          펴는 동안에는 손에 든 것을 감춘다. 손은 하나뿐이다.

          갓은 머리(x 5.4~18.6, 위 끝 y 0.4)를 덮되 사람보다 크지는 않게.
          한때 폭 20.4 로 키웠더니 우산을 쓴 사람이 아니라 우산 아래
          눌린 사람이었고, 반대로 들고 다니던 크기(폭 8.4) 그대로 두었더니
          머리에 겹쳐서 모자챙으로 보였다. 폭 14, 아래 가장자리는 y -2.6 —
          머리 꼭대기보다 위에 있어야 비를 막는 것으로 보인다.

          대는 오른손(18.2, 19.8)에서 올라온다. 갓을 머리 한가운데
...
/*
 * 목도리와 우산은 그 사람의 색을 그대로 쓴다.
 *
 * 무대 색(accent)으로 칠해 봤더니 사람마다 다르던 것이 갑자기
 * 다 같은 금색이 되어, 덧입은 것이 아니라 무대에서 나눠 준
 * 소품처럼 보였다. 옷·모자와 같은 색이라야 그 사람 것이다.
 *
 * 그래도 몸에 묻히지 않는 건, 장비 색이 옷보다 어둡기 때문이다.
 */
.ink .gear.warm {
  stroke-width: 0.9;
}
